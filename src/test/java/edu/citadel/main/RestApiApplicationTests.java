...
@Import(edu.citadel.config.TestSecurityConfig.class)
public class RestApiApplicationTests {
  ...
  public void testListDeleteExist() throws Exception {
    ...
    // Extract the list ID from the response. No need to parse JSON, just grab from
    // the JSON string directly.
    int startIndex = response.indexOf("\"id\":") + 5;
    int endIndex = response.indexOf(",", startIndex);
    String listId = response.substring(startIndex, endIndex);

    // TODO: Once we can set list properties in the post endpoint, test properties
  ...
  public void testUpdateListItem() throws Exception {
    ...
    mockMvc.perform(MockMvcRequestBuilders.patch("/list/" + listId + "/item/" + listItemId)
        ...
        .andExpect(
            MockMvcResultMatchers.jsonPath("$.listItemDesc").value("This item should be associated with the list."));
  }
